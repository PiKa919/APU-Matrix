...
function getSeries(processorName) {
  if (/^Snapdragon\s+8(?:s)?(?:\s+Elite)?\b/i.test(processorName) || /^Dimensity\s+9\d{3}/i.test(processorName) || /^Apple\s+[AM]\d+/i.test(processorName) || /^Tensor\b/i.test(processorName) || /^Exynos\s+2\d{3}/i.test(processorName)) {
    return 'flagship';
  }
  ...
  if (/^Snapdragon\s+4\b/i.test(processorName) || /^Dimensity\s+6\d{3}/i.test(processorName) || /^Unisoc/i.test(processorName) || /^Helio\s+G100\b/i.test(processorName)) {
    return 'entry';
  }

  if (/^Snapdragon\s+6\b/i.test(processorName) || /^Dimensity\s+7\d{3}/i.test(processorName) || /^Kirin\s+8/i.test(processorName) || /^Unisoc\s+T8/i.test(processorName) || /^Helio\s+G\d+/i.test(processorName) || /^Helio\s+P\d+/i.test(processorName)) {
    return 'mid';
  }
