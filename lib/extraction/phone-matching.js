...
const COMPACT_VARIANT_SUFFIXES = ['plus', 'ultra', 'mini', 'lite', 'max', 'pro', 'fe', 'se', 'xl']
...
function splitCompactVariantSuffixes(value) {
  return value
    .split(' ')
    .filter(Boolean)
    .flatMap((token) => {
      if (BRAND_TOKENS.has(token)) {
        return [token]
      }

      for (const suffix of COMPACT_VARIANT_SUFFIXES) {
        if (token.length > suffix.length && token.endsWith(suffix)) {
          return [token.slice(0, -suffix.length), suffix].filter(Boolean)
        }
      }
...
function tokens(value) {
  return splitCompactVariantSuffixes(normalizePhoneName(value))
    .split(' ')
    .filter(Boolean)
