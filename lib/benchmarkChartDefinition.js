...
function pointSignature(point) {
  ...
  return [
    ...
    point.x,
    point.priceInr,
    details.cpuGeekbench6SingleCore,
    details.aiBackend,
    details.aiAccelerator,
    details.aiPrecision,
    details.gpuWildLifeExtremeFps,
    details.processorName,
  ].map((value) => String(value ?? '')).join(':');
}

export function benchmarkChartInputKey(metric, theme = 'dark') {
  const points = sortedPoints(metric?.points || []).map(pointSignature).join('|');
  const series = [...(metric?.series || [])]
    .sort((first, second) => String(first?.id || '').localeCompare(String(second?.id || '')))
    .map((siblingSeries) => `${siblingSeries.id || ''}:${sortedPoints(siblingSeries.points || []).map(pointSignature).join('|')}`)
    .join('|');
...
export function selectPersistentLabels(points = [], series = [], limit = 30) {
  ...
  return ordered.filter((point) => {
    const id = pointId(point);
    if (!selected.has(id) || seen.has(id)) return false;
    seen.add(id);
    return true;
  });
}
...
export function labelPlacement(point, labels = []) {
  const index = labels.findIndex((label) => pointId(label) === pointId(point));
  const lane = (index < 0 ? 0 : index) % 3;
  return [
    { anchor: 'start', dx: 8, dy: -8 },
    { anchor: 'end', dx: -8, dy: 8 },
    { anchor: 'start', dx: 8, dy: 22 },
  ][lane];
}
  ...
  return defineChart({
    marks: [
      ...
      text(labels, {
        ...
        fontSize: 12,
        fontWeight: 600,
        anchor: (point) => labelPlacement(point, labels).anchor,
        dx: (point) => labelPlacement(point, labels).dx,
        dy: (point) => labelPlacement(point, labels).dy,
      }),
    ],
    x: {
      scale: scaleLinear,
